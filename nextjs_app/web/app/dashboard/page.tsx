  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#C7A962]"></div>
      </div>
    );
  }
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-gray-50">
        {/* Dashboard uses the main Navbar component */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link 
                href="/chat/new"
                className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#C7A962] hover:bg-[#B09344] transition-colors duration-200"
              >
                New Chat
              </Link>
            </div>
                  ...
                  <div key={session.id} className="border-b pb-4 last:border-b-0 last:pb-0">
                    <Link href={`/chat/${session.id}`} className="block hover:bg-gray-50 p-2 rounded-md">
                      <div className="flex justify-between items-center">
                        <h3 className="text-lg font-medium text-[#C7A962]">{session.sessionName}</h3>
        ...
        <div className="mt-8 bg-white shadow rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-[#C7A962] mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
